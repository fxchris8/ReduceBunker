                ...
                <table class="table-fixed w-full divide-y divide-gray-200 text-sm text-center rounded border">
                    <thead class="bg-gray-300 sticky top-0 z-10">
                        <tr>
                            ...
                            <th class="px-4 py-2 text-center border-2 border-black">AE Pararel Duration</th>
                            <th class="px-4 py-2 text-center border-2 border-black">Crane Duration</th>
                            <th class="px-4 py-2 text-center border-2 border-black">Manuvering Time</th>
                            <th class="px-4 py-2 text-center border-2 border-black">Differences</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($details as $row)
                            <tr class="text-center odd:bg-white even:bg-gray-200">
                                ...
                                <td class="px-4 py-2 text-center border-2 border-black">{{ number_format($row['ae_pararel'], 2, '.', ',') }}</td>
                                <td class="px-4 py-2 text-center border-2 border-black">{{ number_format($row['crane'], 2, '.', ',') }}</td>
                                <td class="px-4 py-2 text-center border-2 border-black">{{ number_format($row['maneuvering'], 2, '.', ',') }}</td>
                                <td class="px-4 py-2 text-center border-2 border-black">{{ number_format($row['diff'], 2, '.', ',') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
