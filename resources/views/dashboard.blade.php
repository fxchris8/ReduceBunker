                        ...
                        <div class="grid grid-cols-2 gap-4">
                            @foreach($fleetCounts as $fleet => $count)
                                <div class="bg-white p-4 rounded-lg shadow flex flex-col justify-center items-center text-center h-full">
                                    ...
                                    @if($fleet === 'UNKNOWN' && !empty($unknownVessels))
                                        <button 
                                            onclick="document.getElementById('unknownVessels').classList.toggle('hidden')" 
                                            class="mt-4 px-4 py-2 bg-red-600 text-white text-sm font-semibold rounded shadow hover:bg-red-700 transition">
                                            View Detail Unknown Vessels
                                        </button>
                                    @endif
                                </div>
                            @endforeach
                            @if(!empty($unknownVessels))
                                <div id="unknownVessels" class="hidden mt-4 bg-white border border-red-200 rounded-lg shadow-lg p-6">
                                    <h3 class="text-lg font-bold text-red-600 mb-3">Unknown Vessel IDs</h3>
                                    <ul class="space-y-2">
                                        @foreach($unknownVessels as $vesselId)
                                            <li class="px-4 py-2 bg-red-50 border border-red-100 rounded-md text-gray-800">
                                                🚢 Vessel ID: <span class="font-semibold">{{ $vesselId }}</span>
                                            </li>
                                        @endforeach
                                    </ul>
                                </div>
