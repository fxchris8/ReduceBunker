...
<div class="container mx-auto py-6 px-4">
    <div class="bg-white rounded-lg shadow-md">
        <div class="bg-gray-50 px-4 py-4 border-b">
            <div class="flex justify-between items-center"><h1 class="text-xl font-bold">Baseline Analysis Detail</h1><a href="{{ route('po.baseline') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300 transition duration-150 ease-in-out">Back to List</a></div>
        </div>
                ...
                @isset($vessels)
                    <div>
                        ...
                        <select id="vessel" name="vessel"
                            ...
                            onchange="location.href='/baseline-analysis/' + this.value + '?density={{ $density }}'">
                        ...
                        <div class="mt-6">
                            ...
                            <input type="number" step="any" name="density" id="density"
                                value="{{ $density }}"
                                class="block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-600 focus:border-red-600 transition duration-150 ease-in-out"
                                onchange="location.href='{{ route('po.baseline.detail', ['vessel' => $selectedVessel]) }}?density=' + this.value + '&power_kw={{ $power_kw }}&steam_time={{ $steam_time }}'">
                        </div>
                    </div>
                    ...
                    <div>
                        <div class="mt-6">
                            ...
                            <input type="number" step="any" name="power_kw" id="power_kw"
                                value="{{ $power_kw }}"
                                class="block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-600 focus:border-red-600 transition duration-150 ease-in-out"
                                onchange="location.href='{{ route('po.baseline.detail', ['vessel' => $selectedVessel]) }}?density=' + document.getElementById('density').value + '&power_kw=' + this.value + '&steam_time=' + document.getElementById('steam_time').value">
                        </div>
                        ...
                        <div class="mt-6">
                            ...
                            <input type="number" step="any" name="steam_time" id="steam_time"
                                value="{{ $steam_time }}"
                                class="block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-600 focus:border-red-600 transition duration-150 ease-in-out"
                                onchange="location.href='{{ route('po.baseline.detail', ['vessel' => $selectedVessel]) }}?density=' + document.getElementById('density').value + '&power_kw=' + document.getElementById('power_kw').value + '&steam_time=' + this.value">
                        </div>
