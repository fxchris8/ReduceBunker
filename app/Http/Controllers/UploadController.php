...
{   
    function reorderReport($grouped) {
        $ordered = [
            ...
            'BL L/NM' => $grouped['bl_l_nm'] ?? 10,
            'L/NM' => (isset($grouped['steam_time']) && $grouped['steam_time'] > 24 && !empty($grouped['steam_dist'])) ? (($grouped['me_mfo'] ?? 0) / $grouped['steam_dist']) : 0,
            'EXCESS ME MFO L/NM (%)' => (
                isset(
                // $grouped['bl_l_nm'], 
                ...
                ((($grouped['me_mfo'] ?? 0) / $grouped['steam_dist']) > 0)
                    ? ((10 - (($grouped['me_mfo'] ?? 0) / $grouped['steam_dist'])) / 10) * 100
                    : 0
            ) : 0,
